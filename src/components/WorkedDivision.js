...
function WorkedDivision(props) {
  ...
  const handleAssetsChange = (e) => {
    if (e.innerWidth > 1600 && institutionRef.current?.size !== "large") {
      ...
      e.innerWidth <= 1600 &&
      e.innerWidth > 900 &&
      institutionRef.current?.size !== "medium"
    ) {
      institutionRef.current = { size: "medium", perView: 3 };
      setChangeSize("medium");
    } else if (
      e.innerWidth <= 900 &&
      institutionRef.current?.size !== "small"
    ) {
  ...
  useEffect(() => {
    window.addEventListener("resize", (e) =>
      handleAssetsChange(e.currentTarget)
    );
  });
  useEffect(() => {
  ...
  return (
    <Grid
      ...
      <div className="trusted-our-work">
        <h1 className="trusted-title">
          <span className="trusted-bold">{t("home.divisions.trusted")} </span>
          {t("home.divisions.work")}
        </h1>
      </div>

      <div
        className="works-slideshow"
            ...
            {workLogos.map((item, index) => {
              return (
                <li
                  ...
                  <img
                    alt="worked-with-icon"
                    style={{ maxWidth: item.size }}
                    className="work-brands-img"
                    src={item.src}
                  />
                </li>
              );
        ...
        <div
          className="glide__arrows slide-works-arrow-container"
          data-glide-el="controls">
          <span className="works-slideshow-control-arrow" data-glide-dir="<">
            {" "}
            <Icon icon={faChevronLeft} />{" "}
          </span>
          <span className="works-arrow-circle" data-glide-dir="=0">
            <Icon icon={faCircle} />
          </span>
          <span className="works-slideshow-control-arrow" data-glide-dir=">">
            {" "}
